...
Bouw een eenvoudige webapplicatie waarmee een gebruiker trivia-vragen kan ophalen, beantwoorden en laten nakijken,
zonder dat het juiste antwoord vooraf via de client of netwerkresponses zichtbaar wordt.

## Context

De applicatie gebruikt de Open Trivia Database als bron voor trivia-vragen. Omdat deze bron de juiste antwoorden direct
meegeeft in de JSON-respons, moet de oplossing een tussenliggende back-end bevatten die dit afschermt voor de
eindgebruiker.
...
- De response van `GET /questions` moet voldoende gegevens bevatten om de vraag in de UI te tonen en om de gebruiker een
  antwoord te laten kiezen of invullen.
...
- Het systeem moet voorkomen dat de gebruiker via dit proces vooraf het juiste antwoord kan afleiden zonder
  daadwerkelijk een antwoord in te dienen.
...
- Het aantal vragen per ronde, categorieën, moeilijkheidsgraad en scoreberekening zijn niet expliciet gespecificeerd en
  vallen buiten de minimale scope, tenzij later aanvullend gevraagd.
- Een ronde kunnen spelen is MVP, maar de mogelijkheid om opties zoals categorie en moeilijkheid te kiezen is niet
  expliciet gespecificeerd.
- Een ronde kunnen spelen en het keizen van minimaal optie category is MLP, andere opties kiesbaar maken kan in latere
  sprints en vereist dezelfde werkwijze met ENUMS. Dit heeft echter geen impact op het resultaat in de UI Flow. dat wil zeggen, verschillende opties hebben invloed op de request content maar geen invloed op app/UI gedrag
